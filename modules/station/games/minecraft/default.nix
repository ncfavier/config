{ config, pkgs, ... }: let
  version = "1.18.2";
  minecraftHome = "${config.hm.xdg.dataHome}/PrismLauncher/instances/Simply Optimized/.minecraft";
in {
  hm = {
    ...
    home.file."${minecraftHome}/saves".source =
      config.hm.lib.file.mkOutOfStoreSymlink "${config.synced.saves.path}/minecraft";
    home.file."${minecraftHome}/options.txt".source =
      config.lib.meta.mkMutableSymlink ./options.txt;
    home.file."${minecraftHome}/config/sodium-options.json" = {
